...
def parse_battle_result(text):
    ...
    if damage_taken is None:
        print("⚠️ WARNING: [DAMAGE_TAKEN] not found. Using fallback.")
        damage_taken = random.randint(5, 15)

    if damage_dealt is None:
        print("⚠️ WARNING: [DAMAGE_DEALT] not found. Using fallback.")
...
def start_battle_sequence(player, start_index=0):
    ...
    for index, enemy in enumerate(enemies[start_index:], start=start_index):
        ...
        print(f"{player.name}is now at {player.hp} HP!")
